...
import { getAllTreks, MOCK_USER, getOneTrek } from "@/lib/data";
...
export default function PackBuilderPage() {
  ...
  const sherpaReport = useMemo(() => {
    if (!MOCK_TREK) return { isSafe: false, warnings: [], score: 0, breakdown: { equipment: 0, weight: 0, weather: 0, logistics: 0 } } as SherpaReport;
    // Utiliser analyzePackLegacy pour compatibilité avec GearItem[]
    return analyzePack(
      pack.map(item => ({
        id: item.id,
        name: item.name,
        ...
        <div className="max-w-7xl mx-auto px-6 py-6 flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-black text-stone-900 flex items-center gap-3">
              <Icons.NavBuilder className="w-7 h-7 text-orange-600" />
              Pack Builder
              <span className="text-stone-300 text-2xl font-medium">/ {MOCK_TREK?.name || 'Chargement...'}</span>
            </h1>
            <p className="text-sm text-stone-500 mt-1 ml-10">Composez votre sac et vérifiez sa conformité avec le moteur Sherpa.</p>
          </div>
          
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2 text-stone-600 text-sm font-medium bg-stone-50 px-3 py-1.5 rounded-full border border-stone-200">
              <span className={cn(
